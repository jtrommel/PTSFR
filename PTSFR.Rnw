...
\section{Model with trend}\index{trend!regression model}

\subsection{Linear trend}
We can fit a linear model to the data using the \textit{tslm}-function. Instead of working with time as the independent variable, we use the term ''trend"!!!
