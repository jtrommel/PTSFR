...
When we start from the original time-series $O$ with N elements, and we want to study the correlation with $lag=k$ we define:
\begin{eqnarray}
  x_{j} &=& [O_{1}, O_{2}, \ldots , O_{N-k}] \quad  n=(N-k) \\
  y_{j} &=& [O_{k+1}, O_{k+2}, \ldots, O_{N}] \quad n=(N-k)
\end{eqnarray}
...
Rearranging the nominator factors, call the original time-series $O$, the index $i$ and the number of values in the original time series $N$,
\begin{equation}
	r_{k} = \frac{\sum\limits_{i=k+1}^{N}(O_{i-k} - \bar{O})(O_{i}-\bar{O})}{\sum\limits_{i=1}^{N}(O_{i}-\bar{O})^{2}}  = \frac{\sum\limits_{i=k+1}^{N}(O_{i-k} - \bar{O})(O_{i}-\bar{O})}{\sum\limits_{i=1}^{N}(0_{i}-\bar{O})^{2}} = \frac{\sum\limits_{j=1}^{N}(x_{j} - \bar{O})(y_{j}-\bar{O})}{\sum\limits_{i=1}^{N}(0_{i}-\bar{O})^{2}}
\end{equation}

The $\bar{O}$ in the first and second factor of the nominator are the same and refer to the original series. I think that in the first factor $\bar{O}= mean(x_{j})$ for $j=1 \ldots (N-k)$, while in the second factor $\bar{O}= mean(y_{j})$ for $j=1 \ldots (N-k)$. With this interpretation the formula for $r_{k}$ becomes:
\begin{equation}
	r_{k} = \frac{\sum\limits_{j=1}^{N}(x_{j} - \bar{x})(y_{j}-\bar{y})}{\sum\limits_{i=1}^{N}(0_{i}-\bar{O})^{2}}
\end{equation}

in the denominator I expect to find the product of the standard deviatons of the ''reduced" time-series $x$ and $y$. However it refers to the original time-series with $N$ elements.



\sidenote{Discussion of ''High-dimensional autocovariance matrices and optimal linear prediction”, Rob J. Hyndman, Electronic Journal of Statistics Vol. 9 (2015) 792–796 ISSN: 1935-7524 DOI: 10.1214/14-EJS953}
