...
p1 <- ggplot(data = construct, aes(x = t)) +
  geom_line(aes(y = trend*(1+season1)), size=1, color="red") +
  labs(title="") +
  JT.theme
p2 <- ggplot(data = construct, aes(x = t)) +
  geom_line(aes(y = trend*(1+0.05*season1)), size=1, color="red") +
  labs(title="") +
  JT.theme
grid.arrange(p1, p2, nrow=1)
@

But the multiplication means that $y(t)$ will always be equal to zero whenever one of the factors is zero. Let's look at some combinations of characteristics:
\begin{itemize}
  \item trend*season: this changes the amplitude of the seasonal component but it loses the overall trend (Figure~\ref{fig:trend})
    \begin{marginfigure}[0cm]
      ...
      \setfloatalignment{b}
    \end{marginfigure}
  \item trend*(1+season) (Figure~\ref{fig:season})
  \item trend*(1+0.05*season) (Figure~\ref{fig:season})
  \begin{marginfigure}[0cm]
...
\begin{equation}
y(t) = trend*(1 + 0.05*(error + season1 + season2))
\end{equation}
...
ggplot(data = construct, aes(x = t)) +
  geom_line(aes(y = trend*(1 + 0.05*(error + season1 + season2))), size=1, color="red") +
  labs(title="") +
  JT.theme
@
