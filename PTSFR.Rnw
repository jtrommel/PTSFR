...
The \textbf{forecast}-package has the \textit{Acf}-function (Autocorrelation function). For the first 24 datapoints we get:

<<label=AmtrakAcf, fig=TRUE, include=FALSE, echo=FALSE>>=
ridership.24.ts <- window(ridership.ts, start = c(1991, 1), end = c(1991, 24))
Acf(ridership.24.ts, lag.max = 12, main = "Amtrak correlation coefficient for different lags")
@

\begin{figure}
\includegraphics[width=0.85\textwidth]{PTSFR-AmtrakAcf}
\caption{Autocorrelation for Amtrak Ridership}
\label{fig:AmtrakAcf}
\setfloatalignment{b}
\end{figure}
