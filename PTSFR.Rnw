...
Some metrics (e.g. MAPE and RMSE) inflate the effect of large errors. But the effects, or costs, of the error should also come into consideration. Sometimes the cost of a forecast error is big for positive errors, but not for negative ones. Sometimes the cost is proportional to the error, but the relation can be non-linear: it can be that the cost is the same once a certain error treshold is crossed.A measure of profitability could be very useful when choosing the forecast method.
...
We can see that the residuals are \textbf{not} stationary: there is substantial autocorrelation at lags 1, 2 and 3. This tells us that useful information is still available within the residuals which can help to improve out forecast.
...
<<>>=
linmod.res.AR1 <- lm(linmod.res.ARy ~ linmod.res.ARx)
@
...
<<>>=
train.JT$improv.AR1[1] <- 0
train.JT$improv.AR1[2:nTrain] <- predict(linmod.res.AR1)
train.JT$pred.AR1 <- train.JT$pred + train.JT$improv.AR1
@
...
ggplot(data = train.JT, aes(x = t)) +
  geom_line(aes(y = number), lty = 2) +
  geom_line(aes(y = pred), col="red") +
  geom_line(aes(y = pred.AR1), col = "blue") +
  geom_text(x = 25, y = 2110, label = "values", hjust = 0) +
  geom_text(x = 25, y = 2060, color = "red", label = "quadratic + season", hjust = 0) +
  geom_text(x = 25, y = 2010, color = "blue", label = "quadratic + season + AR1", hjust = 0) +
...
\newpage
When we calculate the accuracy metrics for the predictions before and after the AR1-improvement, we can see that it is better:
\medskip
<<>>=
accuracy(train.JT$pred, train.JT$number)
accuracy(train.JT$pred.AR1, train.JT$number)
@
\medskip
The autocorrelation for the residus of the model improved with AR1 are given in Figure~\ref{fig:AmatrakAcfAR1} and we can conclude that the AR1-improvement is sufficient.
<<label=AmatrakAcfAR1, fig=TRUE, include=FALSE, echo=FALSE>>=
train.JT$resid.AR1 <- train.JT$number - train.JT$pred.AR1
Acf(train.JT$resid.AR1, lag.max = 12)
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTSFR-AmatrakAcfAR1}
\caption{Autorregression of Amtrak Ridership with AR1}
\label{fig:AmatrakAcfAR1}
\setfloatalignment{b}
\end{marginfigure}

\newthought{Validation}

<<echo=FALSE>>=
valid.JT <- data.frame(t = seq((nTrain + 1), (nTrain + nValid)), number = Amtrak.data$Ridership[(nTrain + 1):(nTrain + nValid)], season = rep(c("Apr", "May","Jun","Jul","Aug","Sep","Oct","Nov","Dec", "Jan","Feb","Mar"), length.out = nValid))
valid.JT$season <- factor(valid.JT$season, levels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"))
#
# Use the linear model (quadratic + season) based on the training set (linmod) to make predictions in the validaton period
#
newdata <- data.frame(t = valid.JT$t, season = valid.JT$season)
valid.JT$pred <- predict(linmod, newdata)
#
# Calculate the residuals in the validation period
#
valid.JT$resid <- valid.JT$number - valid.JT$pred
#
# Construct the data set of lag1 residuals: the first comes from the training set, the rest from the validation set
#
linmod.res.ARx <- train.JT$resid[nTrain]
linmod.res.ARx[2:nValid] <- valid.JT$resid[1:(nValid - 1)]
#
# Predict the errors bases on an AR1 model
#
valid.JT$improv.AR1 <- predict(linmod.res.AR1, as.data.frame(linmod.res.ARx))
#
# Calculate the improved predictions for the validation period
#
valid.JT$pred.AR1 <- valid.JT$pred + valid.JT$improv.AR1
@

<<label=Amatrakmodelvalid, fig=TRUE, include=FALSE, echo=FALSE>>=
ggplot(data = valid.JT, aes(x = t)) +
  geom_line(aes(y = number), lty = 2) +
  geom_line(aes(y = pred), col="red") +
  geom_line(aes(y = pred.AR1), col="blue") +
  geom_text(x = 129, y = 1700, label = "values", hjust = 0) +
  geom_text(x = 140, y = 2310, color = "red", label = "quadratic + season", hjust = 0) +
  geom_text(x = 134, y = 1740, color = "blue", label = "quadratic + season + AR1", hjust = 0) +
  labs(title = "Validation set: values, prediction, improved AR1") +
  JT.theme
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTSFR-Amatrakmodelvalid}
\caption{Amtrak Ridership: values and global model in validation period}
\label{fig:Amatrakmodelvalid}
\setfloatalignment{b}
\end{marginfigure}

\newthought{Forecasting}

\textbf{But} it is clear that this method is problematic for forecasting, because it depends on knowing the residuals, and that depends on knowing the actual value of $y_{t}$. For future values these are clearly not available. 

For the first step into the future we can calculate two forecasts: $Fg_{t+1}$ based on the global model (quadratic + season) and $Fi_{t+1}$, the improved forecast based on the AR1-model and using the (known) last error of the validation set $e_{t}=y_{t}-Fg_{t}$.

For the second step into the future, we cannot use $e_{t+1}$ because we do not have the value $y_{t+1}$. As a substitue we could use $e'_{t+1}=Fi_{t+1} - Fg_{t+1}$. But this is not very satisfactory because it builds a forecast on a forecast.

<<echo=FALSE>>=
nFuture <- nValid
future.JT <- data.frame(t = seq((nTrain +nValid + 1), (nTrain + nValid + nFuture)), season = rep(c("Apr", "May","Jun","Jul","Aug","Sep","Oct","Nov","Dec", "Jan","Feb","Mar"), length.out = nFuture))
future.JT$season <- factor(future.JT$season, levels = c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"))
#
# Use the linear model (quadratic + season) based on the training set (linmod) to make predictions in the future period
#
newdata <- data.frame(t = future.JT$t, season = future.JT$season)
future.JT$pred <- predict(linmod, newdata)
#
# First step into the future
#
# Use the last known exact residue
#
linmod.res.ARx <- valid.JT$resid[nValid]
#
# Predict the error bases on an AR1 model
#
future.JT$improv.AR1 <- 0
future.JT$improv.AR1[1] <- predict(linmod.res.AR1, as.data.frame(linmod.res.ARx))
#
# Calculate the improved predictions for the validation period
#
future.JT$pred.AR1 <- 0
future.JT$pred.AR1[1] <- future.JT$pred[1] + future.JT$improv.AR1[1]
#
# Step 2 and following
#
for (i in c(2:nFuture)){
  linmod.res.ARx <- future.JT$pred.AR[i-1] - future.JT$pred[i-1]
  future.JT$improv.AR1[i] <- predict(linmod.res.AR1, as.data.frame(linmod.res.ARx))
  future.JT$pred.AR1[i] <- future.JT$pred[i] + future.JT$improv.AR1[i]
}
@

<<label=Amatrakmodelfuture, fig=TRUE, include=FALSE, echo=FALSE>>=
ggplot(data = future.JT, aes(x = t)) +
  geom_line(aes(y = pred), col="#FF000075", size = 2) +
  geom_line(aes(y = pred.AR1), col="blue") +
  geom_text(x = 164, y = 2600, color = "red", label = "quadratic + season", hjust = 0) +
  geom_text(x = 160, y = 2200, color = "blue", label = "quadratic + season + AR1", hjust = 0) +
  labs(title = "Future: values, prediction, improved AR1") +
  JT.theme
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTSFR-Amatrakmodelfuture}
\caption{Amtrak Ridership: values and global model in the future}
\label{fig:Amatrakmodelfuture}
\setfloatalignment{b}
\end{marginfigure}

As we can see the improvement does not last: after a few steps it falls in step with the global forecast. \textbf{Conclusion}: AR-models are great for the training and validation set, but not very useful for forecasting! Sigh.

\newpage
\section{ARIMA models}\index{ARIMA}\index{model!ARIMA}\index{autocorrelation model!ARIMA}\index{AutoRegressive Integrated Moving Average}

\emph{ARIMA}-model stands for \emph{AutoRegressive Integrated Moving Average}-model. It directly models the autocorrelation of the series values as well as the autocorrelation of the forecast errors:

\begin{equation}
	y_{t} = \left[ b_{0} + b_{1}y_{t-1} + \ldots b_{p}y_{t-p} \right ] + \left [ \theta_{1}e_{t-1} + \ldots \theta_{q}e_{t-q} \right]
\end{equation}

To get an ARIMA-model the user has to set a number of parameters:
\begin{enumerate}
	\item $p$: order of the AR-model based on the $y$-values (AR-part)
	\item $q$: order of the AR-model for the forecast errors (MA-part)
	\item $d$: the number of lag1 differencing that should be applied to the original time-series for removing the trend (I-part)
\end{enumerate}

Because the calculations are based on former values of $y_{t}$ (both in the AR-part and, indirectly in the MA-part) this is not a very useful forecating method! However: we can throw in our lot with ARIMA and let the function do an ''autoARIMA"\index{ARIMA!autoARIMA}\index{autoARIMA}. This allows you to make forecasts. But how it works is not clear! 

<<label=Amatrakautoarima, fig=TRUE, include=FALSE, echo=FALSE>>=
Amtrak.auto <- auto.arima(Amtrak.data$Ridership)
futurVal <- forecast(Amtrak.auto, h = 36, level=0.95)
ggplot() +
  geom_line(aes(x = Amtrak.data$t, y = Amtrak.data$Ridership)) +
  geom_line(aes(x = Amtrak.data$t, y = Amtrak.auto$fitted), col = "red") +
  geom_line(aes(x = seq(160, 195), y = futurVal$mean), col = "blue") +
  labs(title = "Results and forecast with autoarima") +
  JT.theme
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTSFR-Amatrakautoarima}
\caption{Amtrak Ridership: forecast with autoarima}
\label{fig:Amatrakautoarima}
\setfloatalignment{b}
\end{marginfigure}

\section{Evaluating predictability}\index{predictability}

\emph{Predictability} means: does the past of a time-series predict the future in an non trivial way? We can measure \emph{predictability} by comparing it to a special case: the \emph{random walk}\index{random walk}. This is a special case of an AR(1) model where $b_{1}=1$:

\begin{equation}
	y_{t} = \beta_{0} + y_{t-1} + \epsilon_{t}
\end{equation}

The difference $y_{t} - y_{t-1}=\beta_{0} + \epsilon$ is random, due to the nature of the $\epsilon$-term. The best forecast available is the \emph{naive forecast}\index{naive forecast}, because there simply is no other information we can use.

\newthought{Example: Amtrak Ridership}
\medskip
To test wether a time-series is a random walk, we apply the \textit{Acf}-function to the differenced (lag1) time series (Figure~\ref{fig:Amatrakcheckpred}). If the difference is random, it should not have important autocorrelations for lags 1, 2, 3. We can see that the autocorrelations are above the treshhold for lag1 an lag2. The time-series is not a random walk! (High autocorrelations for larger lags indicate the seasonal component).

<<label=Amatrakcheckpred, fig=TRUE, include=FALSE, echo=FALSE>>=
Acf(diff(Amtrak.data$Ridership, lag = 1))
@

\begin{marginfigure}
\includegraphics[width=1\textwidth]{PTSFR-Amatrakcheckpred}
\caption{Amtrak Ridership: checking equivalence to random walk}
\label{fig:Amatrakcheckpred}
\setfloatalignment{b}
\end{marginfigure}

We can also fit an AR(1) model and test the hypotesis that the slope co\"{e}fficient is equal to 1 ($H_{0}: \beta_{1} =1$ vs. $H_{1}: \beta_{1} \neq 1$).

We have the AR(1)-model for the training period. Its summary is:
<<>>=
summary(linmod.res.AR1)
@
\medskip
The value of the slope is \Sexpr{round(linmod.res.AR1$coefficients[2],3)}, with a standard error equal to \Sexpr{round(sqrt(diag(vcov(linmod.res.AR1)))[2], 3)}. The difference with a $\beta_{1}=1$ is \Sexpr{round(1 - linmod.res.AR1$coefficients[2], 3)} which is \Sexpr{round((1 - linmod.res.AR1$coefficients[2])/(sqrt(diag(vcov(linmod.res.AR1)))[2]),3)} times the standard error. This makes it highly improbable that the original series acts like a random walk.

\section{Including external information}\index{external information}

\subsection{Outliers}\index{outliers}

Extreme values that can influence regression models considerably. In this case it is sensible to make one regression model \emph{with} the outliers present and another \emph{without} the outliers. If
\begin{itemize}
	\item there is no marked difference between these models: keep them
	\item if there is a marked difference: remove them and fill in the blanks with \emph{imputed}\index{imputing values} values if necessary. Some methods such as regression can work when there are ''NA"'s present (e.g. regression), some cannot (e.g. smoothing). You can also create a new dummy variable ''Outlier" that gets the values $0$ or $1$ and can be incorporated as a factor into the regression.
\end{itemize}

\subsection{Special events}\index{special events}

\emph{Special events} such as holidays (which may not follow the Gregorian calendar!), sport events, concerts ... can have a great influence on the time-series values at particular point in time. We can use the same techniques as with \emph{outliers}.

\subsection{Interventions}\index{interventions}
