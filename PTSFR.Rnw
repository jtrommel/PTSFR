...
Let's try a sine-function with period $T=20$ (Figure~\ref{fig:CorandAfcsin}):
...
\caption{Autoregression of sine}
\label{fig:CorandAcfsin}
\setfloatalignment{b}
\end{marginfigure}

Now a sine-function with period $T=20$ and additive $N(0,\sigma=0.1)$ random error (Figure~\ref{fig:CorandAfcsinanderr}):

<<label=CorandAcfsinanderr, fig=TRUE, include=FALSE, echo=FALSE>>=
res <- data.frame(k = 0, Pearson = 0, rHyndman = 0, Acf = 0)
t <- seq(0, 100)
T <- 20
set.seed(2019)
Orig <- sin(2*pi*t/T) + rnorm(101, 0, 0.1)
for (k in (0:25)) {
  res[(k+1),] <- JT.Hyndman(k, Orig)
}
ggplot(data = res) +
...
\begin{marginfigure}[0cm]
\includegraphics[width=0.85\textwidth]{PTSFR-CorandAcfsinanderr}
\caption{Autoregression of sine + error}
\label{fig:CorandAcfsinanderr}
\setfloatalignment{b}
\end{marginfigure}
