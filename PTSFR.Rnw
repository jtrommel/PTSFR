...
This time series is stored in the data frame \textit{construct} in column \textit{addcon}. It is also stored as an Excel-spreadsheet (\textif{addcon.xlsx}).
...
\begin{itemize}
	...
	\begin{enumerate}
      ...
      \begin{marginfigure}[0cm]
        \includegraphics[width=0.85\textwidth]{PTSFR-Amtrakagg}
        \caption{Amtrak}
        \label{fig:Amtrakagg}
        \setfloatalignment{b}
      \end{marginfigure}
