...
#
additive <- data.frame(t = seq(1:N),
                        error = rnorm(N, mean = mu, sd = sigma),
                        level = rep(level, N))
additive %>% mutate(trend = (b0 + b1*t +b2*t^2) - level) -> additive
additive %>% mutate(season1 = amp1*sin(2*pi*t/T1 + teta1)) -> additive
additive %>% mutate(season2 = amp2*sin(2*pi*t/T2 + teta2)) -> additive
@

<<label=additive, fig=TRUE, include=FALSE, echo=FALSE>>=
p1 <- ggplot(data = additive, aes(x = t)) +
  geom_line(aes(y = error), size = 1, color="red") +
  geom_hline(yintercept = mu, linetype = 2) +
  scale_y_continuous(limits= c(-25, 75)) +
  JT.theme
p2 <- ggplot(data = additive, aes(x = t)) +
  geom_line(aes(y = error + level), size = 1, color="red") +
  geom_hline(yintercept = level, linetype = 2) +
  scale_y_continuous(limits= c(-25, 75)) +
  JT.theme
p3 <- ggplot(data = additive, aes(x = t)) +
  geom_line(aes(y = error + level + trend), size = 1, color="red") +
  geom_hline(yintercept = level, linetype = 2) +
  scale_y_continuous(limits= c(-25, 75)) +
  JT.theme
p4 <- ggplot(data = additive, aes(x = t)) +
