...
This time series is stored in the data frame \textit{construct} in column \textit{addcon}.
...
p2 <- ggplot(data=construct, aes(x=t)) +
  geom_line(aes(y=addcon), size=1, color="red") +
  scale_x_continuous(limits = c(250,500), breaks = seq(250, 500, by=25))  +
...
p3 <- ggplot(data=construct, aes(x=t)) +
  ...
  scale_y_continuous(limits = c(-15,15), breaks = seq(-15, 15, by=2))  +
  labs(title="Additive constructed model\nrange = 275 - 285") +
  JT.theme
grid.arrange(p1, p2, p3, nrow=3)
@
...
\setfloatalignment{b}
\end{figure}
First we plot the whole range. Then we determine the range where we can see a few periods of a possible seasonal. Then we go in for some more detail(Figure~\ref{fig:addconviz}).
...
First we plot the whole range. Then we determine the range where we can see a few periods of a possible seasonal. Then we go in for some more detail (Figure~\ref{fig:combiconviz})
