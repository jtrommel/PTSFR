...
<<label=Validationperiod, fig=TRUE, include=FALSE, echo=TRUE>>=
nValid <- 36
...
abline(v = 1991 + (nTrain + 1)/12 , col="blue")
...
arrows(x0 = 1991 + (nTrain + 1)/12, y0 = 2300, 
...
The \emph{forecast error}\index{forecast!error} or \emph{residual}\index{residual} is the difference between the actual value $y_{t}$ and the forecast value $F_{t}$:
\begin{equation}
	e_{t}=y_{t} - F_{t}
\end{equation}
...
or we immediately use the \textit{accuracy}-function:
<<>>=
