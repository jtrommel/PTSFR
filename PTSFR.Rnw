...
But or a short and/or special time-series the situation is different. Take a simple linear time-series with 10 values ($seq(1,10)$):
...
The same simple linear time-series but now with 100 values ($seq(1,100)$):
...
The Hyndman formula seems to ''punish" lag-series with fewer terms.
